...
public class DuplicateHandler {
	public static void removeDuplicateAndWriteOutputFile(String fileNameAndPath) throws Exception {
		...
		File outputFile = new File(Configuration.FILE_PATH+File.separator+Configuration.OUTPUT_FILE_NAME + Configuration.FILE_EXTENSION);
        ...
        while ((line = bufferedReader.readLine()) != null) {
        	if(line.length()>0) {
	            if(!DatasetCompressor.getEmployeeIdFromCompressedRecord(line).equals(DatasetCompressor.getEmployeeIdFromCompressedRecord(previousLine))) {
	            	bufferedWriter.write(line);
	            	numberOfTuples++;
	            	bufferedWriter.newLine();
	            	previousLine = line;
